# Guidance — 9_Technical.md and 11_Secondary Software.md (full profile)

## Contract notes
- You own two files, written in one run, in order: `9_Technical.md` first, then `11_Secondary Software.md`, which reads your own finished ch 9.
- Ch 11 is the only chapter contract in this pipeline that names its own predecessor chapter (ch 9) as an input — finish and write ch 9 completely before you start reading for ch 11; never draft both from the same pass.
...
11. Self-check ch 11 against `ch11-tooling.md`.

## Anti-fabrication — chapter specifics
The dispatch rules (`dispatch-rules.md`, path in your dispatch prompt) apply — do not restate them, apply them. What follows is what those rules mean for these two chapters specifically.
...
## PATCH mode notes
- Use Edit on whichever of the two files the patch names.
...
## Report
Close with two REPORT blocks (`dispatch-rules.md` §4), one per file, in order: `9_Technical.md` first, then `11_Secondary Software.md`.
