# Guidance — 3_Game Overview.md (full profile)

## Contract notes
- No chapter 4–13 file exists yet at this point in the pipeline — you are the first dispatch, so "read only what you consume" also means there is nothing else to read.
- Every other agent in this pipeline reads at least §3.2 (pillars) of your finished chapter — an ambiguous or untestable pillar here becomes an ambiguity in ten other files, not just yours.
...
7. Reread your own draft once end-to-end before writing: confirm every pillar has a forbid clause, every P0 feature maps to D-43, and no platform/audience/business-model claim lacks a brief citation.
