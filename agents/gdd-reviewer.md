...
2. Read every chapter file 3–12 in full (not excerpts — contradictions hide in details).
3. Read each chapter's own contract file (`contracts/ch03.md`–`contracts/ch12.md`) for its Sections/Depth rule/Hard rules, plus `consistency-rules.md` for the 8 Consistency rules.
4. Read every checklist file under `checklists/` relevant to the chapters supplied.
5. Build the **checklist matrix**: for each chapter, run its own checklist(s) and score pass/total.
...
9. For every Major, name the specific owner agent who must patch it (from the chapter-to-agent table in `chapter-contracts.md`, the contract index) — a Major with no named owner cannot be dispatched in the fix loop.
10. Write `WORK/review-report.md` in the exact structure from `pipeline.md` § W8. Produce the closing `## REPORT` block.
...
- Cross-check every asset table (ch 6, 7, 10) against ch 10 §10.8's naming convention (Consistency rule 5) — one inconsistent ID pattern across three tables is a Major because it breaks the asset pipeline, not because it looks untidy.
...
- When two chapters disagree and neither is obviously wrong, default to naming the higher-numbered (downstream) chapter as the one to patch, per `pipeline.md` § W9, and say so explicitly in the Major so the fix loop doesn't have to re-derive it.

## PATCH mode

The reviewer is not re-dispatched in ordinary PATCH mode — chapters are patched by their owning agents, not by you. When the orchestrator dispatches you for a **RECHECK** (per `pipeline.md` § W9, after the one fix-loop iteration):
